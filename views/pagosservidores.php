    ...
    <section class="container">
        ...
        <div class="row b-3">
            <div class="col-md">
                <form action="" method="post">
                    ...
                    <a href="pagosservidores.php" class="table-btn">Mostrar Todos</a>
                </form>
            </div>
        </div>
        <div class="table-responsive-sm">
            <table class="table table-style">
                ...
                <tbody>
                    ...
                    <?php
                    if (!$_GET) {
                        header('Location:pagosservidores.php?pagina=1');
                    }
                    ...
                    ?>
                        ...
                        if (isset($_POST['busqueda'])) {
                            ...
                            $sql_usuarios = "SELECT * FROM tbladmin WHERE (AdminName LIKE '%$busqueda%') LIMIT :iniciar,:nusuarios";
                            $stm_usuario = $conn->prepare($sql_usuarios);
                            $stm_usuario->bindParam(':iniciar', $iniciar, PDO::PARAM_INT);
                            $stm_usuario->bindParam(':nusuarios', $usuarios_x_pagina, PDO::PARAM_INT);
                            $stm_usuario->execute();
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                ...
                "><a class="page-link" href="pagosservidores.php?pagina=<?php echo $_GET['pagina'] - 1; ?>">Anterior</a></li>

                <?php for ($i = 0; $i < $paginas; $i++) : ?>
                    <li class="page-item pnum <?php echo $_GET['pagina'] == $i + 1 ? ' active' : '' ?>"><a class="page-link" href="pagosservidores.php?pagina=<?php echo $i + 1; ?>"><?php echo $i + 1; ?></a></li>
                <?php endfor  ?>


                <li class="page-item
                <?php echo $_GET['pagina'] >= $paginas ? ' disabled' : '' ?> 
                "><a class="page-link" href="pagosservidores.php?pagina=<?php echo $_GET['pagina'] + 1; ?>">Siguiente</a></li>
            </ul>
        </nav>
    </section>
